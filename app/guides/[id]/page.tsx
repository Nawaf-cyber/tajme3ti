...
export default async function GuideDetails({
  ...
  // لو الرابط دليل لكن المحتوى خبر، حوّله لمساره الصحيح (تفادي محتوى مكرّر)
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0B1120] pb-20 transition-colors duration-300 overflow-x-hidden selection:bg-emerald-500/20">
      ...
      <div className="relative w-full h-[50vh] md:h-[65vh] bg-slate-900 border-b border-slate-200 dark:border-slate-800/80 flex items-end">
        ...
        <div className="absolute top-6 right-4 sm:right-6 lg:right-12 z-20">
          <Link href="/guides" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-sm bg-white/80 dark:bg-[#0B1120]/80 backdrop-blur-md border border-slate-200/50 dark:border-slate-700/50 text-slate-700 dark:text-slate-300 hover:text-emerald-600 dark:hover:text-emerald-400 hover:border-emerald-500/50 transition-all shadow-lg group">
            ...
            <span className="font-bold text-sm">كل الأدلّة</span>
          </Link>
        </div>
        ...
        <div className="relative z-10 w-full max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-32 md:pb-40">
          ...
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-black text-slate-900 dark:text-white leading-[1.3] drop-shadow-xl">
            {article.title}
          </h1>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20 -mt-24 md:-mt-32">
        <article className="bg-white dark:bg-[#0F172A] rounded-sm p-6 md:p-12 lg:p-14 shadow-2xl shadow-slate-200/50 dark:shadow-black/50 border border-slate-200 dark:border-slate-800/80">
          ...
          <div className="mb-10">
            <AuthorByline date={article.createdAt} />
          </div>

          <div className="text-base md:text-lg text-slate-700 dark:text-slate-300 leading-loose md:leading-[2.4] font-medium whitespace-pre-wrap break-words selection:bg-emerald-50 selection:text-emerald-900 dark:selection:bg-emerald-900/40 dark:selection:text-emerald-100">
            {article.content}
          </div>
