...
const USES = [
  ...
  /* «مكتبي» كان معرّفاً في USE_PROFILE وله ثلاثة فروع في هذا الملف
     (يتخطّى سؤال الدقة وسؤال البثّ) — وينقصه زرٌّ واحد. فكان كوداً ميّتاً
     يحرس قيمةً لا تصل، وحاجةً حقيقية بلا مدخل. */
  { key: 'office',              icon: '💼', title: 'مكتبي ودراسة', desc: 'أوفيس · تصفّح · دراسة' },
...
export default function IntentPicker({
  ...
  return (
    <div className="mb-6 rounded-sm border border-cyan-500/30 bg-gradient-to-l from-cyan-500/[0.06] to-transparent dark:from-cyan-950/30 overflow-hidden">
      {/* الرأس */}
      <button
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <span className="w-9 h-9 shrink-0 rounded-sm bg-gradient-to-br from-cyan-500 to-blue-600 flex items-center justify-center text-white shadow-lg shadow-cyan-500/30">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              ...
              {/* ⚠️ أبيضُ على `cyan-500` يعطي 2.37:1 — أسوأ تباينٍ في الصفحة.
                  وcyan-600 يعطي 3.62، وهو دون AA أيضاً. والحلّ حبرٌ داكن على
                  السيان نفسه (6.11) — وهو ما تستعمله بطاقات «تصفّح القطع». */}
              <span className="text-[12px] font-black px-1.5 py-0.5 rounded-sm bg-cyan-500 text-[#052e37]">جديد</span>
            </div>
            <p className="text-[12px] text-slate-500 dark:text-slate-400 font-medium mt-0.5 truncate">
        ...
        <div className="px-4 pb-4">
          ...
          <div className="mb-4">
            <div className="flex items-center gap-2 mb-2.5">
              <span className="w-5 h-5 rounded-sm bg-cyan-500 text-white text-[12px] font-black flex items-center justify-center">١</span>
              <span className="text-[12px] font-black text-slate-600 dark:text-slate-300">وش تسوي بالجهاز؟</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {USES.map(u => (
                <button
                  ...
                  className={`p-3 rounded-sm border text-right transition-all hover:-translate-y-0.5 ${
                    use === u.key
                ...
                >
                  ...
                  <div className="text-[12px] font-medium text-slate-500 dark:text-slate-300 mt-0.5 truncate">
                    {u.desc}
                  </div>
                </button>
              ))}
          ...
          {needsResolution && (
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-2.5">
                <span className="w-5 h-5 rounded-sm bg-cyan-500 text-white text-[12px] font-black flex items-center justify-center">٢</span>
                <span className="text-[12px] font-black text-slate-600 dark:text-slate-300">على أي دقة تلعب؟</span>
              </div>
              <div className="grid grid-cols-3 gap-2">
                {RESOLUTIONS.map(r => (
                  <button
                    ...
                    className={`p-2.5 rounded-sm border text-center transition-all hover:-translate-y-0.5 ${
                      resolution === r.key
                        ? 'border-cyan-500 bg-cyan-500/[0.08] ring-2 ring-cyan-500/20'
                        : 'border-slate-200 dark:border-slate-700 hover:border-cyan-500/50'
                    }`}
                  >
                    ...
                    <div className="text-[12px] font-medium text-slate-500 dark:text-slate-300 mt-0.5">{r.desc}</div>
                  </button>
                ))}
              </div>
            </div>
          ...
          {use && (
            <div className="mb-4">
              <div className="flex items-center gap-2 mb-2.5">
                <span className="w-5 h-5 rounded-sm bg-slate-400 dark:bg-slate-600 text-white text-[12px] font-black flex items-center justify-center">{needsResolution ? '٣' : '٢'}</span>
                <span className="text-[12px] font-black text-slate-600 dark:text-slate-300">تفضّل شركة معيّنة؟</span>
                <span className="text-[12px] font-bold text-slate-500 dark:text-slate-300">(اختياري — اتركه ونختار لك الأفضل)</span>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {/* كرت الشاشة */}
                <div>
                  <div className="text-[12px] font-black text-slate-500 dark:text-slate-300 mb-1.5">كرت الشاشة</div>
                  <div className="flex flex-wrap gap-1.5">
                    {GPU_BRANDS.map(b => (
                      <button
                        ...
                        title={b.hint}
                        className={`px-2.5 py-1.5 rounded-sm border text-[12px] font-bold transition-all ${
                          gpuBrand === b.key
                ...
                {/* المعالج */}
                <div>
                  <div className="text-[12px] font-black text-slate-500 dark:text-slate-300 mb-1.5">المعالج</div>
                  <div className="flex flex-wrap gap-1.5">
                    {CPU_BRANDS.map(b => (
                      <button
                        key={b.label}
              ...
              {gpuBrand === 'Intel' && (resolution === '4K' || resolution === '1440p') && (
                <p className="mt-2 text-[12px] font-bold text-amber-600 dark:text-amber-400">
          ...
          {canBuild && (
            <button
              onClick={() => handleBuild()}
              className="w-full py-3 bg-gradient-to-r from-cyan-500 to-blue-600 text-white text-sm font-black rounded-sm hover:opacity-90 transition-opacity"
            >
              ابنِ لي تجميعة ←
            </button>
          )}
          ...
          {error && (
            <p className="mt-3 text-[12px] font-bold text-red-500 text-center">{error}</p>
          )}

          <p className="mt-3 text-[12px] text-slate-500 dark:text-slate-300 font-medium text-center">
